(function () {
  ...
  function _handleRuntimeMessage(msgType, data, payload) {
    ...
    if (msgType === "getUserInfo" || msgType === "getAccount" || msgType === "getLoginInfo") return gmv("userInfo", null);
    ...
    if (msgType === "getSubscription" || msgType === "getSubscriptionInfo") return gmv("subscriptionInfo", null);
  ...
  function _installLocalStorageBridge() {
    ...
    localStorage.setItem = function(key, value) {
      ...
      if (isImportantKey(key)) {
        ...
        if ((key === "immersiveTranslateIMT_COMMON_JWT_TOKEN" || key === "immersiveTranslateGoogleAccessToken") && typeof value === "string" && value.length >= 16) smv("authToken", value);
      }
    };
      ...
      for (var j = 0; j < _importantKeys.length; j++) {
        ...
        if (v) {
          ...
          try {
            ...
            if (typeof v === "string" && v.length >= 16 && (_importantKeys[j] === "immersiveTranslateIMT_COMMON_JWT_TOKEN" || _importantKeys[j] === "immersiveTranslateGoogleAccessToken")) smv("authToken", v);
          } catch (e) {
            if (typeof v === "string" && v.length >= 16 && (_importantKeys[j] === "immersiveTranslateIMT_COMMON_JWT_TOKEN" || _importantKeys[j] === "immersiveTranslateGoogleAccessToken")) smv("authToken", v);
          }
        }
  ...
  function _tryCaptureData(data, kind, generation, configGeneration) {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      if (kind === "auth") {
        ...
        if (item.subscription || item.plan || item.membership || item.vipInfo) _safeStore("subscriptionInfo", item, 128 * 1024);
      } else if (kind === "config") {
  ...
  window.__imt_get_auth_state = function () {
    ...
    return {
      ...
      userInfo: _sanitizeSyncUserInfo(gmv("userInfo", null), true),
    };
  };
