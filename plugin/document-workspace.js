...
async function handoffLocalFileWithCdp(options) {
  ...
  try {
    ...
    const confirmation = await debuggerApi.sendCommand("Runtime.callFunctionOn", {
      objectId: objectId,
      functionDeclaration: "function(expectedName) { var input = this; var assigned = input.files && input.files.length === 1 ? input.files[0].name : ''; input.dispatchEvent(new Event('input', { bubbles: true })); input.dispatchEvent(new Event('change', { bubbles: true })); return new Promise(function(resolve) { var deadline = Date.now() + 1600; function inspect() { var text = document.body && typeof document.body.innerText === 'string' ? document.body.innerText : ''; var state = { name: assigned, visibleFileName: !!expectedName && text.indexOf(expectedName) !== -1, titleFileName: !!expectedName && document.title.indexOf(expectedName) !== -1, navigated: location.href !== " + JSON.stringify(initialUrl) + " }; if (state.visibleFileName || state.titleFileName || state.navigated || Date.now() >= deadline) resolve(state); else setTimeout(inspect, 100); } inspect(); }); }",
