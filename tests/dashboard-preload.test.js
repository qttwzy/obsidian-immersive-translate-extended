...
test("preload implements the Accounts document PKCE bus and advertises its bridge metadata", async () => {
  ...
  assert.deepEqual(JSON.parse(Buffer.from(meta.content, "base64").toString("utf8")), { version: packageJson.version, _imtBridgeVersion: packageJson.version });
...
test("Dashboard exposes authentication only through the sanitized host accessor", () => {
  ...
  runtime.localStorage.setItem("imt-gm-userInfo", JSON.stringify({ email: "stale@example.com", userType: "pro" }));
  assert.deepEqual(JSON.parse(JSON.stringify(runtime.__imt_get_auth_state())), {
    version: 1,
    authenticated: false,
    token: "",
...
test("response capture requires a trusted endpoint, a session token, and an actual user identity", async () => {
  const runtime = createRuntime({
    fetchResponse: (url) => {
  ...
  runtime.localStorage.setItem("imt-gm-authToken", JSON.stringify("capture-token"));
...
test("sync snapshot omits account profile when the Dashboard session has no token", () => {
  const runtime = createRuntime();
  runtime.localStorage.setItem("imt-gm-userInfo", JSON.stringify({ email: "stale@example.com", userType: "pro" }));
  runtime.localStorage.setItem("imt-gm-subscriptionInfo", JSON.stringify({ plan: "pro" }));
  runtime.localStorage.setItem("imt-gm-fullLocalUserConfig", JSON.stringify({ targetLanguage: "zh-CN" }));
  const snapshot = runtime.__imt_build_sync_snapshot();
  assert.equal(snapshot.values["imt-gm-userInfo"], undefined);
  assert.equal(snapshot.values["imt-gm-subscriptionInfo"], undefined);
  assert.ok(snapshot.values["imt-gm-fullLocalUserConfig"]);
});
